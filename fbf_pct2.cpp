...
static char usage[] = "usage: fbf_pct2 (-debug) (-premium) (-not_premium) filename\n";
...
int main(int argc,char **argv)
{
  ...
  for ( ; ; ) {
    ...
    total_hands += hands;
    total_fbf_count += fbf_count;

    if (!hands)
      dwork = (double)0;
    else
      dwork = (double)fbf_count / (double)hands * (double)100;

    printf("%6.2lf%% (folded before flop on %d of %d hands) %s\n",dwork,fbf_count,hands,filename);
  }
  ...
  if (!total_hands)
    ...
    dwork = (double)total_fbf_count / (double)total_hands * (double)100;

  printf("\n%6.2lf%% (folded before flop on %d of %d total_hands)\n",dwork,total_fbf_count,total_hands);
