...
static char usage[] = "usage: ff_pct2 (-debug) (-premium) (-not_premium) filename\n";

static char ff_str[] = ", ff";
#define FF_STR_LEN (sizeof (ff_str) - 1)
...
int main(int argc,char **argv)
{
  ...
  for ( ; ; ) {
    ...
    total_hands += hands;
    total_ff_count += ff_count;

    if (!hands)
      dwork = (double)0;
    else
      dwork = (double)ff_count / (double)hands * (double)100;

    printf("%6.2lf%% (folded on the flop on %d of %d hands) %s\n",dwork,ff_count,hands,filename);
  }
  ...
  if (!total_hands)
    ...
    dwork = (double)total_ff_count / (double)total_hands * (double)100;

  printf("\n%6.2lf%% (folded on the flop on %d of %d total_hands)\n",dwork,total_ff_count,total_hands);
