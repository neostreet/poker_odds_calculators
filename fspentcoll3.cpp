...
static char usage[] =
"usage: fspentcoll3 (-terse) (-verbose) (-debug) (-hand_typehand_type) (-handhand)\n"
    ...
    for ( ; ; ) {
      ...
      if (Contains(true,
        ...
        if (Contains(true,
          ...
          if ((!bSkipZero || (spent_this_hand != 0)) && (!bSkipFolded || bOnlyFolded)) {
                ...
                if (!bSawRiver || bHaveRiver) {
                  if (!bSpentMoneyOnTheRiver || bSpentRiverMoney) {
                    if (!bHandTypeSpecified || !strcmp(hand_type,plain_hand_types[poker_hand.GetHandType()])) {
                      if (bTerse)
                        printf("%d %d\n",spent_this_hand,collected_from_pot);
                      else {
                        printf("%s %10d %10d",hole_cards,
                          spent_this_hand,collected_from_pot);
        ...
        if (!strncmp(line,flop,FLOP_LEN)) {
          ...
          if ((!bSkipZero || (spent_this_hand != 0)) && (!bOnlyFolded)) {
            if (!bSawFlop || bHaveFlop) {
              if (!bStealthTwoPair || bHaveStealthTwoPair) {
                if (!bSawRiver || bHaveRiver) {
                  if (!bSpentMoneyOnTheRiver || bSpentRiverMoney) {
