...
static char usage[] =
"usage: heads_up_quick (-debug) (-verbose) (-compare_low) filename hands_and_types_filename";
...
int main(int argc,char **argv)
{
  ...
  for ( ; ; ) {
    ...
    for (r = 0; r < POKER_48_5_PERMUTATIONS; r++) {
      ...
      hand[0] = holdem_hand[0].BestPokerHandQuick(hands_and_types,bDebug);
      hand[1] = holdem_hand[1].BestPokerHandQuick(hands_and_types,bDebug);

      if (!bCompareLow)
        ret_compare = hand[0].CompareQuick(hand[1],0,bDebug,hands_and_types);
      else
