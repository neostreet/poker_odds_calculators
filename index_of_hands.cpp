...
static char usage[] =
"usage: index_of_hands (-coalesce) (-card_strings)\n";

static char bad_rank[] = "bad rank in card%d: %s\n";
static char bad_suit[] = "bad suit in card%d: %s\n";

static int index_of_hand(int *cards);
static int coalesced_index_of_hand(int rank_ix1,int suit_ix1,int rank_ix2,int suit_ix2);
...
int main(int argc,char **argv)
{
  ...
  for (curr_arg = 1; curr_arg < argc; curr_arg++) {
    if (!strcmp(argv[curr_arg],"-coalesce"))
      bCoalesce = true;
  ...
  for (cards[0] = 0; cards[0] < NUM_CARDS_IN_DECK - 1; cards[0]++) {
    ...
    for (cards[1] = cards[0] + 1; cards[1] < NUM_CARDS_IN_DECK; cards[1]++) {
      if (!bCoalesce) {
        ...
        ix = coalesced_index_of_hand(rank_ix[0],suit_ix[0],rank_ix[1],suit_ix[1]);
        printf("%3d ",ix);
      }
