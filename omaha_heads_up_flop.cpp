...
#define TWO 2
#define NUM_OMAHA_HEADS_UP_FLOP_CARDS 11
#define NUM_REMAINING_CARDS (NUM_CARDS_IN_DECK - NUM_OMAHA_HEADS_UP_FLOP_CARDS)

#define MAX_LINE_LEN 1024
static char line[MAX_LINE_LEN];

static char usage[] = "usage: omaha_heads_up_flop (-debug) (-verbose) filename";
...
int main(int argc,char **argv)
{
  ...
  int line_no;
  int line_len;
  int cards[NUM_OMAHA_HEADS_UP_FLOP_CARDS];
  int remaining_cards[NUM_REMAINING_CARDS];
  OmahaPokerHand omaha_hand[TWO];
  PokerHand hand[TWO];
  int ret_compare;
  int wins;
  int losses;
  ...
  for ( ; ; ) {
    ...
    for (n = 0; n < NUM_OMAHA_HEADS_UP_FLOP_CARDS; n++) {
      ...
      if (n < NUM_OMAHA_HEADS_UP_FLOP_CARDS - 1) {
        // skip whitespace
    ...
    for (n = 0; n < NUM_CARDS_IN_DECK; n++) {
      for (o = 0; o < NUM_OMAHA_HEADS_UP_FLOP_CARDS; o++) {
        if (n == cards[o])
          break;
      }

      if (o == NUM_OMAHA_HEADS_UP_FLOP_CARDS)
        remaining_cards[m++] = n;
    }
