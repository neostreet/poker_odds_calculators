...
#define NUM_REMAINING_CARDS2 (NUM_CARDS_IN_DECK - NUM_PCT_AT_TURN_CARDS - 2)

#define MAX_LINE_LEN 1024
static char line[MAX_LINE_LEN];

static char usage[] = "usage: pct_at_turn (-debug) (-deep_debug) filename";
...
int main(int argc,char **argv)
{
  ...
  int line_no;
  int line_len;
  int cards[NUM_PCT_AT_TURN_CARDS];
  ...
  for ( ; ; ) {
    ...
    for (n = 0; n < NUM_PCT_AT_TURN_CARDS; n++) {
    ...
    for (n = 0; n < NUM_CARDS_IN_DECK; n++) {
      for (o = 0; o < NUM_PCT_AT_TURN_CARDS; o++) {
        if (n == cards[o])
          break;
      }

      if (o == NUM_PCT_AT_TURN_CARDS)
    ...
    for (outer_o = 0; outer_o < POKER_46_2_PERMUTATIONS; outer_o++) {
      get_permutation_instance1(
