...
#define NUM_STUD_THREE_SIXTH_STREET_CARDS 18
#define NUM_REMAINING_CARDS (NUM_CARDS_IN_DECK - NUM_STUD_THREE_SIXTH_STREET_CARDS)

#define MAX_LINE_LEN 1024
static char line[MAX_LINE_LEN];

static char usage[] = "usage: stud_three_sixth_street (-debug) filename";
...
int main(int argc,char **argv)
{
  ...
  int line_no;
  int line_len;
  int cards[NUM_STUD_THREE_SIXTH_STREET_CARDS];
  ...
  for ( ; ; ) {
    ...
    for (n = 0; n < NUM_STUD_THREE_SIXTH_STREET_CARDS; n++) {
      ...
      if (n < NUM_STUD_THREE_SIXTH_STREET_CARDS - 1) {
        // skip whitespace
    ...
    for (n = 0; n < NUM_CARDS_IN_DECK; n++) {
      for (o = 0; o < NUM_STUD_THREE_SIXTH_STREET_CARDS; o++) {
        if (n == cards[o])
          break;
      }

      if (o == NUM_STUD_THREE_SIXTH_STREET_CARDS)
        remaining_cards[m++] = n;
    }
    ...
    for (m = 0; m < NUM_REMAINING_CARDS; m++) {
      for (q = 0; q < NUM_REMAINING_CARDS; q++) {
        if (q == m)
          continue;

        for (r = 0; r < NUM_REMAINING_CARDS; r++) {
          if ((r == m) || (r == q))
            continue;
          ...
          holdem_hand[1].NewCards(cards[6],cards[7],
            cards[8],cards[9],cards[10],cards[11],
            remaining_cards[q]);

          holdem_hand[2].NewCards(cards[12],cards[13],
