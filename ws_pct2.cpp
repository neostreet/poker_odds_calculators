...
static char usage[] = "usage: ws_pct2 (-debug) (-premium) (-not_premium) filename\n";
...
int main(int argc,char **argv)
{
  ...
  for ( ; ; ) {
    ...
    if (!hands)
      ...
      dwork = (double)ws_count / (double)hands * (double)100;

    printf("%6.2lf%% (won at showdown on %d of %d hands) %s\n",dwork,ws_count,hands,filename);
  }
  ...
  if (!total_hands)
    ...
    dwork = (double)total_ws_count / (double)total_hands * (double)100;

  printf("\n%6.2lf%% (won at showdown on %d of %d total_hands)\n",dwork,total_ws_count,total_hands);
