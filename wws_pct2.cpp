...
static char usage[] = "usage: wws_pct2 (-debug) (-premium) (-not_premium) filename\n";
...
int main(int argc,char **argv)
{
  ...
  for ( ; ; ) {
    ...
    total_hands += hands;
    total_wws_count += wws_count;

    if (!hands)
      dwork = (double)0;
    else
      dwork = (double)wws_count / (double)hands * (double)100;

    printf("%6.2lf%% (won without showdown on %d of %d hands) %s\n",dwork,wws_count,hands,filename);
  }
  ...
  if (!total_hands)
    ...
    dwork = (double)total_wws_count / (double)total_hands * (double)100;

  printf("\n%6.2lf%% (won without showdown on %d of %d total_hands)\n",dwork,total_wws_count,total_hands);
